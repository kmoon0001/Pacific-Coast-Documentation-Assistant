...
export function Sidebar({
  ...
  return (
    <div
      className={cn(
        'border-r border-zinc-100 flex flex-col bg-white transition-all duration-300 h-full',
        isMinimized ? 'w-16 md:w-16' : 'w-64 md:w-64 w-full'
      )}
    >
      <div className="p-3 md:p-5 border-b border-zinc-100">
        <div className="flex items-center justify-between mb-3 md:mb-5">
          ...
          <button
            ...
            className="hidden md:block p-1.5 rounded-lg hover:bg-zinc-100 shrink-0"
          >
            {isMinimized ? (
        ...
        {(!isMinimized || window.innerWidth < 768) && (
          <div className="space-y-2">
            ...
            >
              ...
              {clipboard.length > 0 && (
                <span className="bg-zinc-950 text-white text-[8px] px-1.5 py-0.5 rounded-full shrink-0 ml-1">
                  {clipboard.length}
                </span>
              )}
            </button>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-5 custom-scrollbar" data-tour="history">
        {(!isMinimized || window.innerWidth < 768) && (
          ...
          {history.length === 0
            ...
            : history.map((item, idx) => (
                <div
                  key={idx}
                  className={cn(
                    'p-3 rounded-xl bg-zinc-50 border border-zinc-100 hover:border-zinc-200 transition-all group cursor-default',
                    isMinimized && window.innerWidth >= 768 && 'flex justify-center'
                  )}
                >
                  {isMinimized && window.innerWidth >= 768 ? (
                    ...
                    <>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-[8px] font-medium text-zinc-300 shrink-0 ml-2">
                          {item.timestamp}
                        </span>
                      </div>
                      <p className="text-[10px] font-medium text-zinc-600 line-clamp-2 leading-relaxed">
                        {item.content}
                      </p>
                    </>
                  )}
      ...
      <div className="p-3 md:p-5 bg-zinc-50 border-t border-zinc-100" data-tour="local-mode">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5 min-w-0">
            <ShieldCheck
              className={cn(
                'w-3.5 h-3.5 shrink-0',
                isLocalMode ? 'text-emerald-500' : 'text-zinc-300'
              )}
            />
            {(!isMinimized || window.innerWidth < 768) && (
              <span className="text-[9px] font-black uppercase tracking-widest text-zinc-900 truncate">
                Local Mode
              </span>
            )}
          </div>
            ...
            {modelDownloadProgress !== null && (
              <div className="mt-3">
                <div className="flex justify-between text-[8px] font-black uppercase tracking-widest mb-1">
                  ...
                  <span className="text-zinc-900 shrink-0 ml-2">
